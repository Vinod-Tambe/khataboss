...
const UpdateMoneyLender = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            setLoading(true);
            try {
                const response = await getMoneyLenderByUuid(uuid);
                const ml = response.data;

                if (ml) {
                    setFormData({
                        ml_first_name: ml.ml_first_name || '',
                        ml_last_name: ml.ml_last_name || '',
                        ml_father_name: ml.ml_father_name || '',
                        ml_gender: ml.ml_gender || 'Male',
                        ml_firm_id: ml.ml_firm_id || '',
                        ml_phone: ml.ml_phone || '',
                        ml_email: ml.ml_email || '',
                        ml_dob: ml.ml_dob ? ml.ml_dob.substring(0, 10) : '',
                        ml_aadhaar: ml.ml_aadhaar || '',
                        ml_pan: ml.ml_pan || '',
                        ml_gstin: ml.ml_gstin || '',
                        ml_tax_no: ml.ml_tax_no || '',
                        ml_bank_name: ml.ml_bank_name || '',
                        ml_account_number: ml.ml_account_number || '',
                        ml_ifsc: ml.ml_ifsc || '',
                        ml_branch: ml.ml_branch || '',
                        ml_address: ml.ml_address || '',
                        ml_village: ml.ml_village || '',
                        ml_city: ml.ml_city || '',
                        ml_state: ml.ml_state || '',
                        ml_country: ml.ml_country || '',
                        ml_pincode: ml.ml_pincode || '',
                        ml_notes: ml.ml_notes || '',
                        photo: null,
                        adhaarFront: null,
                        adhaarBack: null,
                        panCard: null,
                    ...
                    if (ml.ml_profile_img?.path) setPhotoPreview(`${baseUrl}${ml.ml_profile_img.path}`);
                    if (ml.ml_adhaar_front_img?.path) setAadhaarFrontPreview(`${baseUrl}${ml.ml_adhaar_front_img.path}`);
                    if (ml.ml_adhaar_back_img?.path) setAadhaarBackPreview(`${baseUrl}${ml.ml_adhaar_back_img.path}`);
                    if (ml.ml_pan_img?.path) setPanCardPreview(`${baseUrl}${ml.ml_pan_img.path}`);
                }
            } catch (error) {
                console.error("Error fetching data:", error);
                toast.error("Failed to load money lender details");
            } finally {
                setLoading(false);
            }
        };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const result = await updateMoneyLender(uuid, data);
            toast.success(result.message || 'Money Lender updated successfully!');
            navigate('/money-lender/list');

        } catch (error) {
            toast.error(error.message || 'Error updating money lender');
        } finally {
            setSaving(false);
        }
    };
    ...
    const renderStep1 = () => (
        <>
            ...
            <div className="row g-3 mb-4">
                ...
                <hr className="my-3" />
                <h5 className="mb-3 mt-0">Additional Details</h5>
                <div className="col-12 col-md-6 col-lg-3">
        ...
        if (!isMobile) {
            return (
                <div className="card p-4 shadow-sm">
                    <h4 className="card-title text-center fw-bold pb-md-0">Update Money Lender</h4>
                    {renderStep1()}
                    {renderStep2()}
                    <div className="d-grid gap-2 col-12 mx-auto mt-5">
                        <button type="submit" className="btn btn-primary btn-lg" disabled={saving}>
                            {saving ? 'UPDATING...' : 'UPDATE'} <i className="bi bi-check-circle ms-2"></i>
                        </button>
                    </div>
                </div>
            );
...
export default UpdateMoneyLender;
