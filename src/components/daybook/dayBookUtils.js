...
export const calculateDayBookSummary = (DayBookData = {}, opening_data = {}) => {
  ...
  const total_today_cash_in_amt = parseFloat(Finance_emi_deposit_data.total_cash_amt || 0);
  const total_today_bank_in_amt = parseFloat(Finance_emi_deposit_data.total_bank_amt || 0);
  const total_today_online_in_amt = parseFloat(Finance_emi_deposit_data.total_online_amt || 0);
  const total_today_card_in_amt = parseFloat(Finance_emi_deposit_data.total_card_amt || 0);
  const total_today_disc_in_amt = parseFloat(Finance_emi_deposit_data.total_disc_amt || 0);
  const total_today_in_amt = parseFloat(Finance_emi_deposit_data.total_amt || 0);
